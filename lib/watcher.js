    ...
    process.on('SIGTERM', (function(self) {
        return function() {
            ...
            for (var index in config.commandsOnEnd) {
                var command = parseCommand(config.commandsOnEnd[index].command);
                var arguments = parseArguments(config.commandsOnEnd[index].command);
...
var setupCommandsOnStart = function(config) {
    for (var index in config.commandsOnStart) {
        var command = spawnCommand(config.commandsOnStart[index].command);
...
var spawnCommand = function(commandString) {
    ...
    if (command) {
        console.log(colors.green('Spawning command:'), command);
